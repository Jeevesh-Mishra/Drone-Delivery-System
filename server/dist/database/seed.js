...
const seedDatabase = async (standalone = false) => {
    ...
    await Drone_1.Drone.deleteMany({});
    await Warehouse_1.Warehouse.deleteMany({});
    await Destination_1.Destination.deleteMany({});
    await NoFlyZone_1.NoFlyZone.deleteMany({});
    await Delivery_1.Delivery.deleteMany({});
    await Package_1.Package.deleteMany({});
    await DeliveryHistory_1.DeliveryHistory.deleteMany({});
    await AnalyticsSnapshot_1.AnalyticsSnapshot.deleteMany({});
    await DashboardMetrics_1.DashboardMetrics.deleteMany({});
    console.log('Collections cleared.');
    // 1. Seed Users
    console.log('Seeding pilot users...');
    const salt = await bcryptjs_1.default.genSalt(10);
    const passwordHash = await bcryptjs_1.default.hash('password123', salt);
    const defaultUser = await User_1.User.create({
        name: 'Arjun Verma',
    ...
    const warehouses = await Warehouse_1.Warehouse.create([
        {
            warehouseId: 'WH-A',
    ...
    const destinations = await Destination_1.Destination.create([
        {
            destinationId: 'C-01',
    ...
    const drones = await Drone_1.Drone.create([
        {
            droneId: 'D-07',
    ...
    const noFlyZones = await NoFlyZone_1.NoFlyZone.create([
        {
    ...
    const d1 = await Delivery_1.Delivery.create({
        deliveryId: 'PKG-2031',
    ...
    const d2 = await Delivery_1.Delivery.create({
        deliveryId: 'PKG-2032',
    ...
    const d3 = await Delivery_1.Delivery.create({
        deliveryId: 'PKG-2033',
    ...
    await AnalyticsSnapshot_1.AnalyticsSnapshot.create(snapshots);
    ...
    await DashboardMetrics_1.DashboardMetrics.create({
        fleetMetrics: {
            totalDrones: 24,
            flyingDrones: 16,
            chargingDrones: 5,
...
if (require.main === module) {
    (0, exports.seedDatabase)(true).catch(err => {
