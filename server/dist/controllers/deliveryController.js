...
exports.dispatchQueue = exports.getQueueState = exports.updateStatus = exports.assignMission = exports.createDelivery = exports.getDeliveries = void 0;
...
const DeliveryHistory_1 = require("../models/DeliveryHistory");
const priorityQueue_1 = require("../algorithms/queue/priorityQueue");
const queue_1 = require("../algorithms/queue/queue");
...
const getDeliveries = async (req, res, next) => {
    try {
        ...
        let list = await Delivery_1.Delivery.find(query).sort({ createdAt: -1 }).lean();
        // Attach packages info to each delivery record
        const listWithPackages = await Promise.all(list.map(async (d) => {
            const pkg = await Package_1.Package.findOne({ deliveryId: d.deliveryId }).lean();
            const history = await DeliveryHistory_1.DeliveryHistory.find({ deliveryId: d.deliveryId })
                .sort({ timestamp: -1 })
                .lean();
            return {
                ...d,
        ...
        if (q) {
            ...
            filteredList = listWithPackages.filter(d => d.deliveryId.toLowerCase().includes(searchStr) ||
...
const createDelivery = async (req, res, next) => {
    try {
        ...
        await DeliveryHistory_1.DeliveryHistory.create({
            deliveryId,
            status: 'Pending',
...
const assignMission = async (req, res, next) => {
    try {
        ...
        const delivery = await Delivery_1.Delivery.findOne({ deliveryId });
        if (!delivery) {
            return res.status(404).json({ success: false, message: `Delivery manifest ${deliveryId} not found.` });
        }
        const drone = await Drone_1.Drone.findOne({ droneId });
        if (!drone) {
    ...
    try {
        ...
        if (status === 'In Transit') {
            ...
            if (delivery.assignedDrone) {
                const drone = await Drone_1.Drone.findOne({ droneId: delivery.assignedDrone });
                if (drone) {
                    // Find route to get final coordinates
                    const route = await Route_1.Route.findOne({ routeId: delivery.assignedRoute });
...
const getQueueState = async (req, res, next) => {
    try {
        const pendingDeliveries = await Delivery_1.Delivery.find({ status: 'Pending' }).lean();
        // 1. Build Standard FIFO Queue
        const fifoQueue = new queue_1.Queue();
...
const dispatchQueue = async (req, res, next) => {
    try {
        // 1. Load pending manifests and available drones
        const pending = await Delivery_1.Delivery.find({ status: 'Pending' }).lean();
        const availableDrones = await Drone_1.Drone.find({ status: 'Available' });
        if (pending.length === 0) {
        ...
        let route = await Route_1.Route.findOne({
        ...
        if (!route) {
            ...
            const wh = await Warehouse_1.Warehouse.findOne({ warehouseId: targetDelivery.warehouseId });
            const dest = await Destination_1.Destination.findOne({ destinationId: targetDelivery.destinationId });
            if (wh && dest) {
                route = await Route_1.Route.create({
        ...
        const deliveryDoc = await Delivery_1.Delivery.findOne({ deliveryId: targetDelivery.deliveryId });
        if (deliveryDoc) {
